...
function stampa_pf($mese,$anno)
{
	...
	$conn2=mysqli_connect($myhost, $myuser, $mypass)
		or die("Connessione non riuscita".mysqli_error($conn2));
	...
	$result=mysqli_query($conn2, $query) or die($query."<br>".mysqli_error($conn2));
...
function stampa_riepilogo($mese,$anno,$format="pdf")
{
	...
	$query="SELECT count(giorno) AS conta FROM presenze WHERE id=".$_SESSION["id"]." AND year(giorno)=$anno AND month(giorno)=$mese AND festivo=0";
	$result=mysqli_query($conn, $query) or die($query."<br>".mysqli_error($conn));
	...
	while($utente=mysqli_fetch_assoc($result))
	{
		...
		if(count($tabella))
		{
			...
			foreach($tabella as $giorno=>$row)
			{
				...
				if(strlen($row["__hidden__trasferta"])>2)
				{
